<%@page import="br.com.dogcatroom.dto.AtendimentoDTO"%>
<%@page import="br.com.dogcatroom.dto.ServicoDTO"%>
<%@page import="java.util.ArrayList"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@ page import="br.com.dogcatroom.dto.AtendimentoDTO"%>
<%@ page import="br.com.dogcatroom.dto.ClienteDTO"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Realizar Atendimento</title>
</head>
<body>

	<form action="AtendimentoController" method="post">
		<table>
			<tr>
				<td>
					<label>ID: </label>
				</td>
				<td>
					<input readonly="readonly" type="text" name="id" value="" />
				</td>
			</tr>
			<tr>
				<td>
					<label>Cliente:	</label>
				</td>
				<td>
					<input readonly="readonly" type="text" name="cliente" value="" />
				</td>
			</tr>
			<tr>
				<td>
					<label>Serviço: </label>
				</td>
				<td>
					<input readonly="readonly" type="text" name="servico" value="" />
				</td>
			</tr>
			<tr>
				<td colspan=2 style="text-align: center">
					<input type="submit" value="Salvar" />
				</td>
			</tr>
		</table>
	</form>

</body>
</html>